...
const translations = {
  es: {
    ...
    "form.sent": "¡Mensaje enviado! Te contactamos pronto.",
  ...
  en: {
    ...
    "form.sent": "Message sent! We'll be in touch soon.",
...
function handleSubmit(btn) {
  ...
  btn.style.background = '#1A5F96';
  btn.disabled = true;

  const serviceId = 'service_nmga4na';
  const templateId = 'template_sij9aaq';
  const publicKey = 'v5tGCGDwUVwLMGb1D';
  ...
  const payloadText = service ? `Servicio: ${service}\n\nMensaje:\n${message}` : message;

  // EmailJS via CDN (required in index.html)
  if (typeof window.emailjs === 'undefined') {
    console.error('EmailJS no está cargado. Asegúrate de incluir el script de emailjs en index.html.');
    alert('Error: EmailJS no está disponible en este navegador.');
    btn.disabled = false;
    return;
  }
  ...
  // Variables esperadas por tu template:
  // - title (llamado "{{title}}")
  // - name ("{{name}}")
  // - email ("{{email}}")
  // - message ("Content")
  window.emailjs
    ...
    .catch((err) => {
      ...
      alert('No se pudo enviar. Revisa configuración/EmailJS.');
    });
}
